...
function infected(s) {
  ...
  const counts = continents.reduce((sum, c)=>{
    sum.population += c.length,
    sum.infected += (c.indexOf('1') !== -1) ? c.length:0
    return sum;
  }, {population:0 , infected:0});

  return (counts.population === 0)? 
    0 : 
    counts.infected / counts.population * 100;
}
