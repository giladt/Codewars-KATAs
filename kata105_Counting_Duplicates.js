...
function duplicateCount(text){
  const duplicatCounter = {};
  text.toLowerCase().split('').forEach(char=>{
    duplicatCounter[char] = (!duplicatCounter[char])? 1 : duplicatCounter[char] + 1;
  });
  
  return Object.values(duplicatCounter).reduce((count,item)=>count+=(item>1)?1:0,0);
}
